#!/usr/bin/env python3
"""
Build comprehensive RFC 6762 requirements database with implementation cross-references.
"""

import re
import json
import os
from pathlib import Path

def parse_rfc_sections(rfc_path):
    """Parse RFC 6762 into structured sections."""
    with open(rfc_path, 'r', encoding='utf-8', errors='replace') as f:
        lines = f.readlines()

    # Remove page headers/footers
    cleaned = []
    for line in lines:
        if 'Cheshire & Krochmal' in line or \
           (line.strip().startswith('RFC 6762') and 'Multicast DNS' in line) or \
           line.strip().startswith('[Page '):
            continue
        cleaned.append(line)

    # Find section boundaries
    section_pattern = re.compile(r'^(\d+(?:\.\d+)*)\.\s+(.+)$')
    sections = []
    current_section = None
    current_lines = []

    for i, line in enumerate(cleaned):
        match = section_pattern.match(line.strip())
        if match:
            # Save previous section
            if current_section:
                sections.append({
                    'number': current_section[0],
                    'title': current_section[1],
                    'text': ''.join(current_lines),
                    'line_start': current_section[2]
                })
            current_section = (match.group(1), match.group(2), i)
            current_lines = []
        elif current_section:
            current_lines.append(line)

    # Add last section
    if current_section:
        sections.append({
            'number': current_section[0],
            'title': current_section[1],
            'text': ''.join(current_lines),
            'line_start': current_section[2]
        })

    return sections

def extract_normative_statements(text, section_num, section_title):
    """Extract all RFC 2119 normative statements from text."""
    requirements = []

    # RFC 2119 keywords with word boundaries
    must_not_pattern = r'\b(MUST NOT|SHALL NOT|REQUIRED NOT)\b'
    must_pattern = r'\b(MUST|SHALL|REQUIRED)\b'
    should_not_pattern = r'\bSHOULD NOT\b'
    should_pattern = r'\b(SHOULD|RECOMMENDED)\b'
    may_pattern = r'\b(MAY|OPTIONAL)\b'

    # Split into sentences (improved)
    # Handle common abbreviations
    text = text.replace('e.g.,', 'e_g_')
    text = text.replace('i.e.,', 'i_e_')
    text = text.replace('etc.', 'etc_')

    sentences = re.split(r'(?<=[.!?])\s+(?=[A-Z])', text)

    for sent in sentences:
        # Restore abbreviations
        sent = sent.replace('e_g_', 'e.g.,')
        sent = sent.replace('i_e_', 'i.e.,')
        sent = sent.replace('etc_', 'etc.')

        # Check for normative keywords
        req_type = None
        if re.search(must_not_pattern, sent):
            req_type = 'MUST NOT'
        elif re.search(must_pattern, sent):
            req_type = 'MUST'
        elif re.search(should_not_pattern, sent):
            req_type = 'SHOULD NOT'
        elif re.search(should_pattern, sent):
            req_type = 'SHOULD'
        elif re.search(may_pattern, sent):
            req_type = 'MAY'

        if req_type:
            # Clean up sentence
            sent = ' '.join(sent.split())
            # Skip very short matches or section references
            if len(sent) > 20 and 'RFC' not in sent[:50]:
                requirements.append({
                    'type': req_type,
                    'text': sent,
                    'section': section_num,
                    'section_title': section_title
                })

    return requirements

def search_implementation(req_text, beacon_root):
    """Search Beacon codebase for implementation of requirement."""
    # Extract key terms from requirement
    key_terms = []

    # Common mDNS terms to search for
    terms_map = {
        'multicast address': ['224.0.0.251', 'FF02::FB', 'multicast'],
        'port 5353': ['5353'],
        'probe': ['Probe', 'probing'],
        'announce': ['Announce', 'announcing'],
        'conflict': ['Conflict', 'conflict'],
        'cache': ['cache', 'Cache'],
        'TTL': ['TTL', 'ttl'],
        'known-answer': ['KnownAnswer', 'known_answer'],
        'unicast-response': ['unicast', 'Unicast'],
        'QU': ['QU', 'unicast-response'],
        'QM': ['QM'],
        'unique': ['unique'],
        'shared': ['shared'],
    }

    for term, searches in terms_map.items():
        if term.lower() in req_text.lower():
            key_terms.extend(searches)

    # Search for implementation
    implementations = []
    tests = []

    # Search in key directories
    search_dirs = [
        'querier',
        'responder',
        'internal/transport',
        'internal/responder',
        'internal/state',
        'internal/records',
        'internal/protocol',
        'internal/message'
    ]

    for dir_name in search_dirs:
        dir_path = os.path.join(beacon_root, dir_name)
        if not os.path.exists(dir_path):
            continue

        for root, dirs, files in os.walk(dir_path):
            for file in files:
                if file.endswith('.go') and not file.endswith('_test.go'):
                    file_path = os.path.join(root, file)
                    try:
                        with open(file_path, 'r') as f:
                            content = f.read()
                            for term in key_terms:
                                if term in content:
                                    rel_path = os.path.relpath(file_path, beacon_root)
                                    if rel_path not in implementations:
                                        implementations.append(rel_path)
                                    break
                    except:
                        pass

                elif file.endswith('_test.go'):
                    file_path = os.path.join(root, file)
                    try:
                        with open(file_path, 'r') as f:
                            content = f.read()
                            for term in key_terms:
                                if term in content:
                                    rel_path = os.path.relpath(file_path, beacon_root)
                                    if rel_path not in tests:
                                        tests.append(rel_path)
                                    break
                    except:
                        pass

    return implementations, tests

def main():
    beacon_root = '/home/user/development/beacon'
    rfc_path = os.path.join(beacon_root, 'RFC Docs/RFC-6762-Multicast-DNS.txt')

    print("Parsing RFC 6762...")
    sections = parse_rfc_sections(rfc_path)

    print(f"Found {len(sections)} sections")

    # Extract requirements from sections 1-10 (as requested)
    all_requirements = []
    req_id = 1

    for section in sections:
        sec_num = section['number']
        # Only process sections 1-10 for now
        if sec_num.split('.')[0].isdigit() and int(sec_num.split('.')[0]) <= 10:
            requirements = extract_normative_statements(
                section['text'],
                sec_num,
                section['title']
            )

            for req in requirements:
                req['id'] = req_id
                req['rfc_id'] = f"RFC6762-§{sec_num}-REQ-{req_id:03d}"
                req_id += 1
                all_requirements.append(req)

    print(f"Extracted {len(all_requirements)} requirements from sections 1-10")

    # Search for implementations
    print("Searching for implementations...")
    for req in all_requirements:
        implementations, tests = search_implementation(req['text'], beacon_root)
        req['implementations'] = implementations
        req['tests'] = tests

        # Determine status
        if implementations and tests:
            req['status'] = 'COMPLETE'
        elif implementations:
            req['status'] = 'PARTIAL'
        else:
            req['status'] = 'MISSING'

        # Set priority
        if 'MUST' in req['type']:
            req['priority'] = 'P0'
        elif 'SHOULD' in req['type']:
            req['priority'] = 'P1'
        else:
            req['priority'] = 'P2'

    # Generate markdown report
    print("Generating report...")

    with open(os.path.join(beacon_root, 'RFC_REQUIREMENTS_SECTIONS_1-10.md'), 'w') as f:
        f.write("# RFC 6762 Requirements Database (Sections 1-10)\n\n")
        f.write(f"**Generated**: 2026-01-06\n\n")

        # Summary statistics
        must_count = len([r for r in all_requirements if 'MUST' in r['type']])
        should_count = len([r for r in all_requirements if 'SHOULD' in r['type'] and 'NOT' not in r['type']])
        may_count = len([r for r in all_requirements if r['type'] == 'MAY'])

        complete_count = len([r for r in all_requirements if r['status'] == 'COMPLETE'])
        partial_count = len([r for r in all_requirements if r['status'] == 'PARTIAL'])
        missing_count = len([r for r in all_requirements if r['status'] == 'MISSING'])

        f.write("## Summary\n\n")
        f.write(f"- **Total Requirements**: {len(all_requirements)}\n")
        f.write(f"  - MUST/MUST NOT: {must_count} (P0)\n")
        f.write(f"  - SHOULD/SHOULD NOT: {should_count} (P1)\n")
        f.write(f"  - MAY/OPTIONAL: {may_count} (P2)\n\n")
        f.write(f"- **Implementation Status**:\n")
        f.write(f"  - ✅ Complete: {complete_count}\n")
        f.write(f"  - ⚠️  Partial: {partial_count}\n")
        f.write(f"  - ❌ Missing: {missing_count}\n\n")

        # Group by section
        sections_dict = {}
        for req in all_requirements:
            sec = req['section']
            if sec not in sections_dict:
                sections_dict[sec] = []
            sections_dict[sec].append(req)

        f.write("## Requirements by Section\n\n")

        for sec in sorted(sections_dict.keys(), key=lambda x: [int(n) for n in x.split('.')]):
            reqs = sections_dict[sec]
            f.write(f"### §{sec} {reqs[0]['section_title']}\n\n")

            for req in reqs:
                status_icon = {'COMPLETE': '✅', 'PARTIAL': '⚠️', 'MISSING': '❌'}[req['status']]
                f.write(f"#### {req['rfc_id']} {status_icon}\n\n")
                f.write(f"- **Type**: {req['type']}\n")
                f.write(f"- **Priority**: {req['priority']}\n")
                f.write(f"- **Status**: {req['status']}\n\n")
                f.write(f"**Requirement**:\n")
                f.write(f"> {req['text']}\n\n")

                if req['implementations']:
                    f.write(f"**Implementation**:\n")
                    for impl in req['implementations']:
                        f.write(f"- `{impl}`\n")
                    f.write("\n")
                else:
                    f.write(f"**Implementation**: NOT IMPLEMENTED\n\n")

                if req['tests']:
                    f.write(f"**Tests**:\n")
                    for test in req['tests']:
                        f.write(f"- `{test}`\n")
                    f.write("\n")
                else:
                    f.write(f"**Tests**: NO TEST\n\n")

                f.write("---\n\n")

    # Save JSON for programmatic access
    with open(os.path.join(beacon_root, 'rfc_requirements_sections_1-10.json'), 'w') as f:
        json.dump(all_requirements, f, indent=2)

    print(f"✅ Generated RFC_REQUIREMENTS_SECTIONS_1-10.md")
    print(f"✅ Generated rfc_requirements_sections_1-10.json")
    print(f"\nSummary:")
    print(f"  Total: {len(all_requirements)}")
    print(f"  MUST: {must_count}")
    print(f"  SHOULD: {should_count}")
    print(f"  MAY: {may_count}")
    print(f"  Complete: {complete_count}")
    print(f"  Partial: {partial_count}")
    print(f"  Missing: {missing_count}")

if __name__ == '__main__':
    main()
